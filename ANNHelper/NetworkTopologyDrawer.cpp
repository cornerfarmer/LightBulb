...
void NetworkTopologyDrawer::refresh()
{
	...
	// Calculate the offset between two layers (The +1 is needed for a border)
	int layerOffset = options->width / (options->networkTopology->getNeurons()->size() + 1);	
	// Go through all layers in the given networkTopology
	int layerIndex = 0;
	for (std::vector<std::vector<AbstractNeuron*>>::iterator layer = options->networkTopology->getNeurons()->begin(); layer != options->networkTopology->getNeurons()->end(); layer++, layerIndex++)
	{
		// Calculate the offset between two neurons (The +1 is needed for a border)
		int neuronOffset = options->height / (layer->size() + 1);
		// Go through all neurons in this layer
		int neuronIndex = 0;
		for (std::vector<AbstractNeuron*>::iterator neuron = layer->begin(); neuron != layer->end(); neuron++, neuronIndex++)
		{
