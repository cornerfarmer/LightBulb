...
BackpropagationLearningRule::BackpropagationLearningRule(int maxIterationsPerTry_, int maxTries_, float totalErrorGoal_, float learningRate_, float minRandomWeightValue_, float maxRandomWeightValue_)
{
					...
					for (std::list<Neuron*>::iterator neuron = neuronsInLayer->begin(); neuron != neuronsInLayer->end(); neuron++, neuronIndex++)
					{
						...
						if (l == 2)
						{
							...
							if ((*errorvector)[neuronIndex] != 0)
							{
								...
								{					
									...
									(*edge)->setWeigt((*edge)->getWeight() + learningRate * (*edge)->getPrevNeuron()->getActivation() * deltaVectorOutputLayer[neuronIndex]);		
								}
							}
						}
						else if (neuronIndex + 1 < neuronsInLayerCount) // If its the second last layer and not a BiasNeuron
						{
							...
							for (std::list<Edge*>::iterator afferentEdge = afferentEdges->begin(); afferentEdge != afferentEdges->end(); afferentEdge++)
							{
								...
								(*afferentEdge)->setWeigt((*afferentEdge)->getWeight() + learningRate * (*afferentEdge)->getPrevNeuron()->getActivation() * (dynamic_cast<StandardNeuron*>(*neuron))->executeDerivationOnActivationFunction((dynamic_cast<StandardNeuron*>(*neuron))->getNetInput()) * nextLayerErrorValueFactor);		
							}
						}
					}
				}
