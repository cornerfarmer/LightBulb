...
namespace LightBulb
{
	...
	private:
		...
		double exponent;
		/**
		 * \brief Determines the maximum euclidean distance between two objects when the fitness value should still be reduced.
		 */
		double dissimilarityThreshold;
	public:
		/**
		 * \brief Creates the fitness sharing function.
		 * \param dissimilarityThreshold_ Determines the maximum euclidean distance between two objects when the fitness value should still be reduced. (Value is problem specific!)
